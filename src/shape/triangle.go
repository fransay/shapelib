...
// NewTriangle initialise a new Triangle object given its sides.
...
func (t *Triangle) Area() (areaBySides float64) {
	...
	areaBySides = math.Sqrt(semiPerimeter * (semiPerimeter - t.SideOne) * (semiPerimeter - t.SideTwo) * (semiPerimeter - t.SideThree))
	return areaBySides
}
...
// InRadius returns the radius of an inner circle
func (t *Triangle) InRadius() float64 {
	return t.Area() / t.SemiPerimeter()
}

// CircumRadius returns the radius of an exterior circle
func (t *Triangle) CircumRadius() float64 {
	return (t.SideOne * t.SideTwo * t.SideThree) / (4 * t.Area())
}
